---
title: "Clustered Recognition of Intermittent Bouts (CRIB)"
...
* **method = "CRIB"** Here we specify that R should run the CRIB method. Right
  now, the only other option is `"Troiano_MVPA"`. More methods can be added in
  the future.
...
* **minimum_bout_epochs = 10** Here we specify that only bouts lasting $\geq10$
...
than `Inf`.) Notably, `minimum_bout_epochs` is a filtering criterion. It has no
direct effect on how the bouts are defined; it simply affects which ones are
...
* **start_index** The starting point of the bout (e.g., `intensity[1022]` for
  the first bout in this example)
* **end_index** The ending point of the bout (e.g., `intensity[1039]` for
  the first bout in this example)
...
As before, epoch length influences the interpretation of length variables. Keep
that in mind.
